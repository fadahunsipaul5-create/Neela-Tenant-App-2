...
const ListingCard: React.FC<{ listing: Listing; handleApply: (listing: Listing) => void }> = ({ listing, handleApply }) => (
  <div className="bg-white rounded-2xl shadow-lg shadow-slate-500/10 border-2 border-slate-200/60 overflow-hidden hover:shadow-2xl hover:shadow-indigo-500/20 transition-all duration-300 flex flex-col transform hover:-translate-y-1 group">
    ...
    <div className="p-7 flex-1 flex flex-col bg-white">
      ...
      {(listing.furnishingType || (listing.furnishingsBreakdown && listing.furnishingsBreakdown.length > 0)) && (
        <div className="mb-4 p-3 bg-indigo-50 rounded-lg border border-indigo-100">
          <p className="text-xs font-semibold text-indigo-700 uppercase tracking-wide mb-1">Furnishing</p>
          <p className="text-sm font-medium text-slate-800">{listing.furnishingType || 'Furnished'}</p>
          {listing.furnishingsBreakdown && listing.furnishingsBreakdown.length > 0 && (
            <p className="text-xs text-slate-600 mt-1">{listing.furnishingsBreakdown.join(' • ')}</p>
          )}
        </div>
      )}
